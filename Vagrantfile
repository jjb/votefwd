...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    su - postgres << POSTGRES
      psql postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='vagrant'" | grep -q 1 || psql -c "create role vagrant with login password 'vagrant' superuser;"
POSTGRES
    su - vagrant << VAGRANT
      echo 'PATH=./node_modules/.bin:$PATH' >> /home/vagrant/.profile
